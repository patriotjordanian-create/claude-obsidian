...
2026-07-12 (2): **Wiki lint complete**. Full health check at `wiki/meta/lint-report-2026-07-12.md`. Two error-tier clusters: (1) 3 concept pages from the 2026-04-24 autoresearch run ([[Persistent Wiki Artifact]], [[Source-First Synthesis]], [[Query-Time Retrieval]]) plus both `wiki/references/` pages are post-rollout but missing DragonScale `address:` fields; (2) 5 pages link `[[How does the LLM Wiki pattern work?]]` with a trailing question mark the filename lacks, so the link never resolves. Also: `wiki/index.md` counts stale (34/2 claimed vs 49/1 actual), ecosystem pages still reflect the April v1.2.0 snapshot (plugin now v1.9.2, most cherry-picks shipped), and this hot cache's own Plugin State block below is stale (v1.7.1 era). Tiling skipped (no ollama here). No fixes applied; recommended fix order is in the report. Awaiting user go.

2026-07-12: **Ingest reconcile session** (remote container, branch `claude/skill-learning-3rs0kb`). User asked to ingest; the vault's only raw source `.raw/claude-obsidian-ecosystem-research.md` turned out to be already fully ingested on 2026-04-08 (all 8 derived pages present and indexed) but unrecorded in `.raw/.manifest.json`, which predated it. Reconciled: manifest `sources` now records the file with hash `72c6e827…` and the full page lists, so delta tracking skips it going forward. Side effects: first run of `scripts/detect-transport.sh` in this container wrote `.vault-meta/transport.json` (preferred: filesystem; no Obsidian CLI or MCP here). No wiki pages created or modified beyond log + hot cache. Repo is now at v1.9.2 public canonical (`cb93ff6`); the v1.7.1 threads below are historical.
